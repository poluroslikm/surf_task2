...
Формирование, расписание и доставка самих push-сообщений — зона ответственности **backend этой же
поставки** (изначально на этапе анализа требований предполагалась «существующая серверная
инфраструктура уведомлений», `00-foundations.md` §8.1; решение реализовать её самим — см.
`6-development/BE_IMPLEMENTATION_PLAN.md` → BE-10); клиентское приложение получает системное
разрешение и передаёт объект подписки через `registerPushSubscription` (`api/push/api.yaml`, см.
раздел «API-запросы» ниже). Резервного канала уведомлений (SMS/email) в продукте нет (Q-014) —
при недоступности push клиент увидит актуальный статус при следующем открытии приложения (FR-22).
...
5. **Регистрация подписки на сервере** — при результате `granted` браузер выдаёт объект подписки
   (`PushSubscription`), который клиент передаёт серверу через `registerPushSubscription`
   (`api/push/api.yaml`), чтобы backend мог присылать push по FR-21/FR-22 (см. раздел
   «API-запросы» ниже).
...
> push-подписки — операция `registerPushSubscription` покрыта контрактом (`api/push/api.yaml`).

### `registerPushSubscription` — решено (см. `api/push/api.yaml`)

**Статус:** закрыто. Ранее числилось открытым вопросом контракта (аналогично тарифу проката в
`api/README.md`) — решение принято в `6-development/FEATURES_IMPLEMENTATION_PLAN.md` → FEAT-01:
пятый домен `api/push/`, операция `POST /push/subscriptions` (upsert по `endpoint`). Доставку
самих push-сообщений (напоминание за 24ч — FR-21, отмена студией — FR-22) реализует backend этой
же поставки (`BE_IMPLEMENTATION_PLAN.md` → BE-10), а не внешняя «существующая инфраструктура»,
как предполагалось на этапе анализа требований.
...
| Результат | Действие |
|-----------|----------|
| `200`, подписка сохранена | Никакого видимого клиенту эффекта — регистрация фоновая, не блокирует и не подтверждает закрытие BS-002 |
