...
- **Дизайн-бриф применён к обоим экранам** (`3-design-brief/00-foundations.md`,
  `SCR-001-auth.md`, `SCR-002-slot-list.md`) — каркас через `Scaffold` (фикс. хедер/CTA, скролл
  между ними, `imePadding()` на SCR-001, чтобы CTA не перекрывался клавиатурой); на SCR-002 —
  порядок полей карточки, разделитель « · » между программой и тегом сложности («Новичковый»/
  «Для опытных»), заголовок «Шеф Имя», хедер с иконками фильтра/аккаунта, чип периода, фикс.
  таб-бар и оба варианта Empty (`«Выбрать другой период»` для дефолтного периода, `«Сбросить
  фильтр»` для применённого) — по ASCII-wireframe и §8 обоих SCR-документов.

## Не реализовано

- **FE-05** — полноценная корневая навигация с таб-баром: сейчас `App.kt` — простое
  `if (isAuthenticated)` переключение Auth↔Slots, без Decompose; вкладка «Мои записи» и иконки
  фильтра/аккаунта на SCR-002 отрисованы по дизайну, но не ведут никуда (`onClick = {}` с
  комментарием) — соответствующих экранов ещё нет.
- **BS-001** — сама шторка выбора даты (UI) не реализована, только контракт стора
  (`ApplyDateFilter`/`ResetDateFilter`); чип периода на SCR-002 показывает необработанный ISO-
  timestamp, а не читаемый формат «3–20 июля» из wireframe — форматирование дат на русском не
  подключено.
