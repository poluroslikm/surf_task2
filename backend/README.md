...
- **BE-06…BE-09** — весь домен `bookings`:
  ...
  - `submitRating` — только для прошедших активных записей без ранее поставленной оценки;
    ...
    диапазон `1–5`.
- **BE-12** — юнит- и HTTP-тесты для всех четырёх доменов (`auth`/`slots`/`bookings`/`push`), плюс
  один интеграционный тест на реальной Postgres (`internal/storage/bookings_repo_reminder_test.go`,
  пропускается без `DATABASE_URL`). Подробности и покрытие — `BE_IMPLEMENTATION_PLAN.md` → BE-12,
  как это было сделано (два субагента) — `promt/тесты.md`.
- **FEAT-01..05 (push-уведомления, FR-21/FR-22, BE-10)** — пятый домен контракта `api/push`
  ...
  фоновый воркер напоминаний за 24ч (тикер 5 мин; см. «Баги, найденные ревью» — окно пересчитано с
  версии на скользящее); dev-only `POST /internal/slots/{slotId}/force-cancel` (заголовок
  `X-Internal-Token`, не часть `api/`) — каскадно переводит слот и его активные брони в
  `cancelled`/`cancelled_by_studio` одной транзакцией с `SELECT ... FOR UPDATE`, шлёт push после
  коммита.
...
**Баги, найденные полным ревью проекта и исправленные** (см. коммит «Fix two bugs found in the
full-project review»):
1. Воркер напоминаний (`internal/worker/reminder.go`) считал скользящее окно
   `[now+24ч-interval, now+24ч)` по интервалу тикера — пропущенный тик или рестарт процесса
   навсегда терял брони, чьё окно попало в простой. Заменено на не хранящий состояние запрос
   «активна, ещё не отправлено напоминание, старт в пределах ближайших 24ч» — переживает любой
   рестарт/простой без пропусков. Подтверждено интеграционным тестом
   (`bookings_repo_reminder_test.go`) и юнит-тестом воркера (`worker/reminder_test.go`).
2. `X-Internal-Token` сравнивался обычным `!=` — тайминг-канал (низкая реальная опасность, dev-only
   эндпоинт). Заменено на `crypto/subtle.ConstantTimeCompare`.
...
- **BE-12 (частично)** — `testcontainers-go` не подключён (единственный DB-тест читает
  `DATABASE_URL` из окружения и пропускается, если не задан); `go test -race` не прогонялся
  отдельно; concurrency-тест для `createBooking` прогнан вживую один раз, но не оформлен как
  повторяемый Go-тест.
...
`go build ./...`, `go vet ./...`, `go test ./...` — чисто на текущем коде, все четыре домена
(`auth`/`slots`/`bookings`/`push`) покрыты автотестами (BE-12); с `DATABASE_URL` в окружении
дополнительно проходит интеграционный тест воркера напоминаний на реальной Postgres.
