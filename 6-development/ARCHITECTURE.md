...
проекта. Web Push (открытый пункт BE-10) при реализации встраивается в тот же backend-процесс, не
выносится отдельным контейнером.
...
- Протокол — REST JSON, `Authorization: Bearer <opaque-token>` на все эндпоинты кроме
  `register`/`login`.
- Контракт — OpenAPI 3.0.3, `api/{auth,slots,bookings,common}/*.yaml` — единственный источник
...
состояний `Booking`/`Slot`, ключевые инварианты); физическая схема — таблицы, `CHECK`-ограничения
и индексы — [`backend/migrations/00001_init.sql`](../backend/migrations/00001_init.sql). Здесь —
только сводка сущностей для целостности архитектурной картины:
...
| `Slot` | read-only для клиента | переход `scheduled → cancelled` — вне клиентского API (BE-10) |
...
- Push-инфраструктура (регистрация подписки, воркер напоминаний, каскадная отмена слота студией)
  — открытый пункт BE-10, требует сначала правки `api/`, не только кода.
