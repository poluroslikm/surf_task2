...
- [x] [FE-05. Корневая навигация и таб-бар](#fe-05-корневая-навигация-и-таб-бар)
...
- [x] [FE-07. Карточка слота SCR-003](#fe-07-карточка-слота-scr-003)
- [x] [FE-08. Оформление записи SCR-004 + подтверждение BS-002 (LOGIC-004)](#fe-08-оформление-записи-scr-004--подтверждение-bs-002-logic-004)
- [x] [FE-09. Мои бронирования SCR-005](#fe-09-мои-бронирования-scr-005)
- [x] [FE-10. Детали брони SCR-006 + отмена BS-003 (LOGIC-002)](#fe-10-детали-брони-scr-006--отмена-bs-003-logic-002)
- [x] [FE-11. Оценка шефа SCR-007](#fe-11-оценка-шефа-scr-007)
- [x] [FE-12. Аккаунт / выход BS-004](#fe-12-аккаунт--выход-bs-004)
...
FE-00, FE-02 (без кодогенерации типов), FE-03, FE-04, FE-05, FE-06 (без BS-001), FE-07, FE-08,
FE-09, FE-10, FE-11, FE-12 сделаны и **реально собраны и протестированы** (`tsc --noEmit` +
`npm run build`, см. `client/README.md` → «Проверено вживую») — в отличие от Kotlin-попытки,
которая не компилировалась вообще. FE-07..FE-12 реализованы двумя параллельными саб-агентами в
изолированных git worktree (по образцу бэкенда — BE-06..09), затем вручную проверены построчно и
слиты в основную ветку.
...
Сделано: `react-router-dom` (`BrowserRouter`/`Routes`), полный маршрутный контракт задокументирован
как комментарий в `App.tsx` (`/`, `/slots/:slotId`, `/slots/:slotId/book`, `/bookings`,
`/bookings/:bookingId`, `/bookings/:bookingId/rate`); `TabBar.tsx` — реальная навигация между
«Классы»/«Мои записи»; иконка аккаунта на `SlotsScreen`/`MyBookingsScreen` открывает реальный
`AccountSheet` (BS-004).

Готово, когда: все маршруты навигируемы, `tsc --noEmit`/`npm run build` проходят — **проверено
вживую**.
...
Сделано: `screens/slotCard/`. Всегда переисполняет `getSlot(slotId)` независимо от списка/кэша
(защита от устаревшего `free_seats` перед записью). Бейдж вместимости — тот же приоритет
«Отменён студией» > «Мест нет» > «Свободно: N из M», что и на `SlotsScreen`. CTA «Записаться»
рендерится только при `status='scheduled' && free_seats>0` (без disabled-варианта).

Готово, когда: экран открывается по `/slots/:slotId`, показывает фото/состав/аллергены/шефа/цену,
`tsc --noEmit`/`npm run build` проходят — **проверено вживую**.

### FE-08. Оформление записи SCR-004 + подтверждение BS-002 (LOGIC-004) ✅

Сделано: `screens/booking/` + `components/BookingSuccessSheet.tsx`. Цена всегда `slot.price`, не
зависит от `equipment_choice`. `createBooking` — по `api-sequence.md` сценарий 1 буквально: `409
slot_full` → снек «Мест больше нет. Список обновлён.» + возврат на список слотов; `410
slot_cancelled` → снек «Класс отменён студией» + возврат на список; `400` — текст сервера;
сеть/5xx — сквозные тексты. Push-разрешение (LOGIC-004) — одноразовый флаг в `localStorage`
(`chef-stol.push_offered`), `Notification.requestPermission()` без блокировки кнопки «К моим
записям»; регистрация push-подписки на сервере не делается (открытый пробел контракта, см.
`api/README.md`).

Готово, когда: полный цикл записи проходит, ошибки обрабатываются по сценарию, `tsc
--noEmit`/`npm run build` проходят — **проверено вживую**.

### FE-09. Мои бронирования SCR-005 ✅

Сделано: `screens/myBookings/`. Группировка по таблице `SCR-005-my-bookings.md` §«Логика статуса и
группировки»: только `status='active'` с `slot.start_at` в будущем → «Предстоящие»; всё остальное
(включая отменённые записи на ещё не начавшийся слот) → «Прошедшие», независимо от даты. Сортировка:
«Предстоящие» — по возрастанию даты, «Прошедшие» — как отдаёт API (по убыванию).

Готово, когда: список открывается по `/bookings`, переключатель групп работает без нового запроса,
`tsc --noEmit`/`npm run build` проходят — **проверено вживую**.

### FE-10. Детали брони SCR-006 + отмена BS-003 (LOGIC-002) ✅

Сделано: `screens/bookingDetails/` + `components/CancelConfirmSheet.tsx`. Превью на BS-003 —
исключительно по серверному `booking.free_cancellation_until` (никогда не пересчитывается из
`slot.start_at - 24h` на клиенте). Финальный статус/снек — только из ответа `cancelBooking`
(«Запись отменена» / «Поздняя отмена: продукты на класс уже закуплены. Штраф не взимается.»).
Шторка закрывается, снек и обновление статуса показывает родительский SCR-006 (не сама шторка).
CTA «Отменить»/«Оценить шефа» — по таблице статусов из спеки.

Готово, когда: отмена проходит по сценарию 2 `api-sequence.md`, `tsc --noEmit`/`npm run build`
проходят — **проверено вживую**.

### FE-11. Оценка шефа SCR-007 ✅

Сделано: `screens/rating/`. Контекст (программа/дата/шеф) приходит через `navigate(..., {state})`
от SCR-006 без лишнего запроса; на прямом заходе/перезагрузке — резервный `getBooking` с
пред-проверкой `already_rated`/`unavailable`. `422 not_ratable`/`409 already_rated` — переводят
экран в соответствующее guard-состояние, без краша формы; редактирование ранее поставленной оценки
не предусмотрено (UC-4 E2).

Готово, когда: оценка отправляется, состояния-заглушки показываются корректно, `tsc
--noEmit`/`npm run build` проходят — **проверено вживую**.

### FE-12. Аккаунт / выход BS-004 ✅

Сделано: `components/AccountSheet.tsx`. Два шага (email read-only → подтверждение), без
backdrop/swipe-закрытия (критическое подтверждение — исключение из общего правила шторок),
`logout` → `session.clear()` → реактивный переход на `AuthScreen`; ошибка логаута — инлайн-текст в
шторке, а не снек.

Готово, когда: `tsc --noEmit`/`npm run build` проходят — **проверено вживую**.
